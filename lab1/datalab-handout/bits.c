 ...
 * use the least bit of x to fill 64 bits
 *   Examples:
...
long remainderPower2(long x, long n) {
    // get symbol bit. if flag equal 0, x is positive,else x is negative
    long flag = x >> 63;
    long mask = 0;
    x = (~flag & x) | (flag & (~x + 1));
    x = x & ((1 << n) + ~mask);
...
long rotateLeft(long x, long n) {
    long mask = ((long)(1) << n) + ~0;
    long cnt = 64;
    long right = (x >> (cnt + ~n + 1)) & mask;
    long left = (x << n) & (~mask);
    return left | right;
}
/*
...
long bitMask(long highbit, long lowbit) {
    long res = ~0;
    long r = 1;
    return (res << lowbit) & (res + (r << highbit << 1));
}
/*
...
long bitCount(long x) {
    ...
    long tmpMask1 = (0x55) | (0x55 << 8);
    long mask1 = (tmpMask1) | (tmpMask1 << 16);
    mask1 = mask1 | (mask1 << 32);
    long tmpMask2 = (0x33) | (0x33 << 8);
    long mask2 = (tmpMask2) | (tmpMask2 << 16);
    mask2 = mask2 | (mask2 << 32);

    long tmpMask3 = (0x0f) | (0x0f << 8);
    long mask3 = (tmpMask3) | (tmpMask3 << 16);
    mask3 = mask3 | (mask3 << 32);

    long mask4 = (0xff) | (0xff << 16);
    mask4 = mask4 | (mask4 << 32);

    long mask5 = (0xff) | (0xff << 8);
    mask5 = mask5 | (mask5 << 32);

    long mask6 = (0xff) | (0xff << 8);
    mask6 = mask6 | (mask6 << 16);
